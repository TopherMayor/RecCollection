...
export const recipeService = {
  ...
  // Get saved recipes
  async getSavedRecipes(params: SearchParams = {}) {
    // Build query string
    const queryParams = new URLSearchParams();
    ...
    return apiClient<RecipeListResponse>(`/recipes/saved${queryString ? `?${queryString}` : ''}`);
  },
  ...
  async saveRecipe(id: number) {
    return apiClient<{ success: boolean }>(`/recipes/${id}/save`, {
      method: 'POST',
    });
  },
