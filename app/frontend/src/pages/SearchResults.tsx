...
export default function SearchResults() {
  ...
  const [activeTab, setActiveTab] = useState<'all' | 'users' | 'recipes'>('all');
  ...
  useEffect(() => {
    ...
    const fetchResults = async () => {
      ...
      try {
        ...
        let searchUrl = `/api/search?q=${encodeURIComponent(query)}&page=${page}`;
        if (type) {
          searchUrl += `&type=${type}`;
        }
  ...
  // Render pagination
  const renderPagination = (pagination: { total: number; page: number; limit: number; totalPages: number }) => {
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="border-b border-gray-200 mb-6">
        <nav className="-mb-px flex space-x-8">
          ...
          >
            Recipes {results?.recipes?.pagination.total ? `(${results.recipes.pagination.total})` : ''}
          </button>
          <button
          ...
          >
            Users {results?.users?.pagination.total ? `(${results.users.pagination.total})` : ''}
          </button>
        </nav>
      </div>
      ...
      {!loading && !error && query && results && (
        (activeTab === 'all' && 
          (!results.users?.items.length && !results.recipes?.items.length)) ||
      ...
      {!loading && !error && results && (
        <div className="space-y-8">
          ...
          {(activeTab === 'all' || activeTab === 'users') && results.users?.items.length > 0 && (
            <div>
              {activeTab === 'all' && <h2 className="text-2xl font-bold mb-4">Users</h2>}
              <div className="space-y-4">
                {results.users.items.map(renderUserCard)}
              </div>
              {activeTab !== 'all' && renderPagination(results.users.pagination)}
            </div>
          )}
          ...
          {(activeTab === 'all' || activeTab === 'recipes') && results.recipes?.items.length > 0 && (
            <div>
              {activeTab === 'all' && <h2 className="text-2xl font-bold mb-4">Recipes</h2>}
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {results.recipes.items.map(renderRecipeCard)}
              </div>
              {activeTab !== 'all' && renderPagination(results.recipes.pagination)}
            </div>
          )}
          
          {/* Pagination for 'all' tab */}
          {activeTab === 'all' && (
            results.users?.pagination.totalPages > 1 || results.recipes?.pagination.totalPages > 1
          ) && (
            <div className="flex justify-center mt-8">
              <button
