...
export class EmailService {
  ...
  constructor() {
    ...
    this.defaultFrom = process.env.EMAIL_FROM || "RecCollection <noreply@reccollection.com>";
  ...
  async sendEmail(data: EmailData): Promise<boolean> {
    try {
      ...
      if (!data.to || !data.subject || !data.text) {
        throw new HTTPException(400, { message: "Invalid email data" });
      }
  ...
  async sendRecipeSharedEmail(
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        ${message ? `<p>Message from ${senderName}:</p><blockquote style="border-left: 3px solid #4F46E5; padding-left: 15px; margin-left: 0;">${message}</blockquote>` : ''}
