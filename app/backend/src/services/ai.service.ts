...
export class AIService {
  ...
  async generateRecipeName(userId: number, ingredients: string[], instructions: string[], category?: string): Promise<string> {
    try {
      ...
      const cookingMethods = ['Grilled', 'Baked', 'Roasted', 'Fried', 'Sautéed', 'Steamed', 'Boiled'];
      ...
      throw new HTTPException(500, { message: 'Failed to generate recipe name' });
    }
  }
  
  // Generate a recipe description based on name, ingredients, and instructions
  async generateRecipeDescription(userId: number, name: string, ingredients: string[], instructions: string[], category?: string): Promise<string> {
    try {
      // In a real implementation, this would call an AI service
      ...
      let description = `This ${category ? category.toLowerCase() + ' ' : ''}recipe features `;
      ...
      // Add a cooking method if we can detect one from instructions
      const cookingMethods = ['grill', 'bake', 'roast', 'fry', 'sauté', 'steam', 'boil'];
      let method = '';
      ...
      await db.insert(schema.aiGenerations).values({
        ...
        inputData: JSON.stringify({ name, ingredients, instructions, category }),
      ...
      throw new HTTPException(500, { message: 'Failed to generate recipe description' });
    }
  }
