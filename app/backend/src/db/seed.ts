  ...
  try {
    ...
    await db.insert(schema.instructions).values([
      ...
      },
      {
        recipeId: recipe1.id,
        stepNumber: 6,
        description: "Stir in the chocolate chips.",
      },
      {
        recipeId: recipe1.id,
        stepNumber: 7,
    ...
    await db.insert(schema.recipeCategories).values([
      {
        recipeId: recipe1.id,
        categoryId: categories.find((c) => c.name === "Dessert")!.id,
      },
    ]);
    ...
    await db.insert(schema.recipeTags).values([
      ...
      {
        recipeId: recipe1.id,
        tagId: tags.find((t) => t.name === "baking")!.id,
      },
    ]);
    ...
    const [recipe2] = await db
      ...
      .values({
        ...
        description:
          "A quick and healthy vegetable stir fry that's perfect for busy weeknights.",
        cookingTime: 10,
        prepTime: 15,
        servingSize: 4,
        difficultyLevel: "easy",
    ...
    await db.insert(schema.ingredients).values(recipe2Ingredients);
    ...
    await db.insert(schema.instructions).values([
      {
        ...
        stepNumber: 1,
        description:
          "Prepare all vegetables: cut broccoli into florets, slice carrots, dice bell peppers, and trim snap peas.",
      },
      {
        recipeId: recipe2.id,
        stepNumber: 2,
      ...
      },
      {
        recipeId: recipe2.id,
        stepNumber: 4,
        description: "Add broccoli and carrots, stir-fry for 2 minutes.",
      },
      {
        recipeId: recipe2.id,
        stepNumber: 5,
        description:
          "Add bell peppers and snap peas, stir-fry for another 2 minutes.",
      },
      {
        recipeId: recipe2.id,
        stepNumber: 6,
        description:
          "Add soy sauce, sesame oil, and red pepper flakes, toss to combine.",
      },
      {
        recipeId: recipe2.id,
        stepNumber: 7,
        description:
          "Continue cooking for 1-2 minutes until vegetables are crisp-tender.",
      },
      {
        recipeId: recipe2.id,
        stepNumber: 8,
        description: "Serve immediately over rice or noodles if desired.",
      },
    ]);
    ...
    await db.insert(schema.recipeCategories).values([
      {
        recipeId: recipe2.id,
        categoryId: categories.find((c) => c.name === "Dinner")!.id,
      },
      {
        recipeId: recipe2.id,
        categoryId: categories.find((c) => c.name === "Vegetarian")!.id,
      },
      {
        recipeId: recipe2.id,
        categoryId: categories.find((c) => c.name === "Quick & Easy")!.id,
      },
    ]);
    ...
    await db.insert(schema.recipeTags).values([
      {
        recipeId: recipe2.id,
        tagId: tags.find((t) => t.name === "healthy")!.id,
      },
      { recipeId: recipe2.id, tagId: tags.find((t) => t.name === "spicy")!.id },
    ]);

    console.log(
      "Created recipes with ingredients, instructions, categories, and tags"
    );
    ...
    await db.insert(schema.comments).values([
      {
        ...
        recipeId: recipe1.id,
        content:
          "These cookies are amazing! I added some walnuts and they turned out great.",
        createdAt: new Date(),
        updatedAt: new Date(),
      },
      {
      ...
      },
      {
        ...
        recipeId: recipe2.id,
        content:
          "I made this for dinner last night and it was delicious. I added some tofu for protein.",
        createdAt: new Date(),
        updatedAt: new Date(),
      },
    ]);
